...
def store_docs_in_dataset3(docs):
    try:
        conn = mysql.connector.connect(
            host="localhost",
            database="ir_docs",
        ...
        if conn.is_connected():
            ...
            for doc in docs:
                cur.execute(
                    "INSERT INTO dataset3 (id, doc_id, title, text) VALUES (%s, %s, %s, %s)", 
                    (count, doc['id'], doc['title'], doc['text'])
                )
                count = count + 1
...
def store_docs_in_dataset4(docs):
    try:
        conn = mysql.connector.connect(
            host="localhost",
            database="ir_docs",
        ...
        if conn.is_connected():
            ...
            for doc in docs:
                cur.execute(
                    "INSERT INTO dataset4 (id, doc_id, text) VALUES (%s, %s, %s)", 
                    (count, doc['id'], doc['text'])
                )
                count = count + 1
